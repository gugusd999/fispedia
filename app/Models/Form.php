...
class Form extends Model
{
    ...
    public static function getfile($dds ,$data, $path)
    {

        $dataf = $_FILES['data'];
        $name = $dataf['name'][$data];
        $filename = uniqid();
        $ext = pathinfo($name, PATHINFO_EXTENSION);
        $type = $dataf['type'][$data];
        $tmp = $dataf['tmp_name'][$data];
        $err = $dataf['error'][$data];
        $size = $dataf['size'][$data];
        if (!file_exists($path)) {
